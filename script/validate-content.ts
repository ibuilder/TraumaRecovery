...
chapters.forEach((chapter, index) => {
  ...
  check(
    ...
    `${where}: duplicate chapter id "${chapter.id}"`
  );
  ...
  check(
    /\d+\s*min read/.test(chapter.readingTime),
    `${where}: odd readingTime "${chapter.readingTime}"`
  );
...
const exportedComponents = [
  ...
  ...chartSource.matchAll(/^export function (\w+)\(\)[\s\S]*?(?=^export |$(?![\s\S]))/gm),
];
...
const manifestPath = new URL("../client/src/lib/chapters/manifest.ts", import.meta.url);
...
const searchIndexPath = new URL("../client/src/lib/search-index.json", import.meta.url);
